...
public class RobotContainer extends DeafultRobotContainer{
  ...
  private void configureBindings() {
    T(StateTrigger.T(() -> (getDriverController().getMiddle()
     || ((getRobotState() == RobotConstants.EJECT || getRobotState() == RobotConstants.SHOOTING) && SuperStructure.getBallCount() == 0))
    ,RobotConstants.IDLE));

    T(StateTrigger.T(() -> (getDriverController().getL1() || getDriverController().getR1()
     || (Magazine.getInstance().getCurrentState() != MagazineConstants.RESET  && SuperStructure.getBallCount() < 5 ))
    ...
    T(StateTrigger.T(() -> (getDriverController().getActionsRight() && SuperStructure.getBallCount() != 0)
