...
public class PoseEstimator {
    ...
    public static Pose2d update() {
        ...
        if (frc.robot.RobotControl.Field.FieldRectangle.contains(currentPose.exp(fused).getTranslation())
                && currentPose.getTranslation().getDistance(currentPose.exp(fused).getTranslation()) < 1) {
    ...
    private static void replayHistory() {
        ...
        if (frc.robot.RobotControl.Field.FieldRectangle.contains(pose.getTranslation())
                && currentPose.getTranslation().getDistance(pose.getTranslation()) < 1) {
            history.clear();
